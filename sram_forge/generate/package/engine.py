...
class PackageEngine:
    ...
    def create_package(
        ...
        self._generate_librelane(
            package_dir, chip_config, sram_spec, slot_spec, fit_result
        )
        ...
        manifest = self.generate_manifest(
            chip_config, sram_spec, fit_result, package_name
        )
    ...
    def _generate_librelane(
        ...
        pdn_cfg = self.librelane_engine.generate_pdn(
            chip_config, sram_spec, slot_spec, fit_result
        )
    ...
    def _generate_docs(
        ...
        datasheet = self.docs_engine.generate_datasheet(
            chip_config, sram_spec, fit_result
        )
        (docs_dir / "datasheet.md").write_text(datasheet)

        memory_map = self.docs_engine.generate_memory_map(
            chip_config, sram_spec, fit_result
        )
    ...
    def _init_git(self, package_dir: Path, chip_config: ChipConfig) -> None:
        ...
        try:
            ...
            commit_msg = (
                f"Initial commit: {chip_config.chip.name}\n\nGenerated by sram-forge"
            )
            subprocess.run(
