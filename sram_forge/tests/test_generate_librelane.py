...
def test_generate_config_has_clock_period(
    librelane_engine, chip_config, sram_spec, slot_spec
):
...
def test_generate_pdn_tcl(librelane_engine, chip_config, sram_spec, slot_spec):
    ...
    assert (
        "pdn" in result.lower() or "power" in result.lower() or "vdd" in result.lower()
    )
...
def test_generate_sdc(librelane_engine, chip_config, sram_spec, slot_spec):
    ...
    assert (
        "create_clock" in result.lower()
        or "set_clock" in result.lower()
        or "period" in result.lower()
    )
